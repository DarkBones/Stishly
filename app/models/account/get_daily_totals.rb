  ...
  class GetDailyTotals
    ...
    def perform
      ...
      @transactions.each do |t|
        ...
        if @account_id != 0
          days[day] = @current_user.transactions.where("DATE(transactions.local_datetime) = DATE(?) AND parent_id IS NULL AND account_id = ? AND is_scheduled = false", day, @account_id).sum(:account_currency_amount) unless days.keys.include? day
        else
          days[day] = @current_user.transactions.where("DATE(transactions.local_datetime) = DATE(?) AND parent_id IS NULL AND is_scheduled = false", day).sum(:user_currency_amount) unless days.keys.include? day
        end
      end
      return days
    end
